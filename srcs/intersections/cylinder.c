...
/*   Updated: 2025/03/12 16:38:03 by eamsalem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	camera_in_cy(t_cy *cylinder, t_camera *camera)
{
    ...
    height_projection = dot(to_camera, cylinder->normal);
    if (height_projection < 0 || height_projection > cylinder->height)
        return false;
    
    radial_vec = sub(to_camera, scale(cylinder->normal, height_projection));
