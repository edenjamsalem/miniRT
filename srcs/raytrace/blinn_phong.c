...
t_rgb	get_spec(t_light *light, t_intsec *intsec, t_vec3 view_dir)
{
	...
	if (specular <= 0)
		return ((t_rgb){0, 0, 0});
	spec_exponent = pow(specular, intsec->surf.exp);
	intensity = light->brightness * intsec->surf.k_specular * spec_exponent;
	return (rgb_scale(light->colour, intensity));
}
...
t_rgb	get_light_i(t_light **lights, t_intsec *intsec, t_vec3 view_dir)
{
	...
	while (lights[++i])
	{
		if (lights[i]->visibility <= 0.0)
			continue ;
		diffuse_i = get_diffuse(lights[i], intsec, intsec->surf.k_diffuse);
...
t_rgb	blinn_phong(t_scene *scene, t_intsec *intsec, t_vec3 view_dir)
{
	...
	ambient_i = get_i_ambient(&scene->ambient_light, intsec->surf.k_ambient);
	if (intsec->in_shadow)
